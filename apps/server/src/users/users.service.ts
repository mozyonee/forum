...
export class UsersService {
  ...
  async followUser(body: { follower: UserDto; followed: UserDto }) {
    ...
    const condition = following.some(
      (follow: mongoose.Types.ObjectId) => follow.toString() === followed._id,
    );

    if (!condition)
      updatedFollowing = [
        ...following,
        new mongoose.Types.ObjectId(followed._id),
      ];
    else
    ...
    follower.following = updatedFollowing.map((follow) =>
      follow.toString(),
    ) as [string];
