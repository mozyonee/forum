...
export class PostsService {
  constructor(
    @InjectModel('Post') private readonly postModel: mongoose.Model<PostDto>,
  ) {}
  ...
  async getPostByID(id: string) {
    const [result] = await this.postModel.aggregate([
      ...
      },
      {
        $addFields: {
          repliesCount: {
            $ifNull: [{ $arrayElemAt: ['$repliesCountData.count', 0] }, 0],
          },
        },
      },
  ...
  async getReplies(parent: string | null) {
    const posts = await this.postModel.aggregate([
      {
        $match: { parent: parent ? new mongoose.Types.ObjectId(parent) : null },
      }, // get replies to post
  ...
  async createPost(body: {
    ...
    if (!author || !(text || attachments.length))
  ...
  async likePost(body: { user: UserDto; post: PostDto }) {
    ...
    const condition = likes.some(
      (liked: mongoose.Types.ObjectId) => liked.toString() === user._id,
    );
  ...
  async repostPost(body: { user: UserDto; post: PostDto }) {
    ...
    const condition = reposts.some(
      (reposted: mongoose.Types.ObjectId) => reposted.toString() === user._id,
    );

    const updatedReposts = condition
      ? reposts.filter(
          (reposted: mongoose.Types.ObjectId) =>
            reposted.toString() !== user._id,
        )
  ...
  async deletePost(body: { post: string }) {
    ...
    const deletePostRecursively = async (id: string | null) => {
      const childPosts = await this.postModel.find({
        parent: id,
      });
