		...
		<>
			{user && (
				<form onSubmit={createPost} className="flex flex-col border border-foreground p-3">
					...
					<div
						className={`flex ${fileStates.length > 0 ? 'justify-between' : 'justify-end'} gap-3 items-end`}
					>
						{fileStates.length > 0 && (
							<div className="flex gap-3">
								{fileStates?.map(({ file, progress }, index) => (
									<div
										key={index}
										className={'p-0 h-16 w-16 relative shadow-md rounded-md aspect-square'}
									>
										<Image
										...
										{imageUrls[index] && progress === 'COMPLETE' && (
											<div
												className="w-full h-full group absolute right-0 top-0 flex justify-center items-center
											bg-transparent hover:bg-black/75 cursor-pointer transition-all duration-250 rounded-md"
												onClick={(event) => {
													event.stopPropagation();
													setFileStates((prevFileStates) =>
														prevFileStates.filter((_, i) => i !== index),
													);
													setUrls((prevUrls) => prevUrls.filter((_, i) => i !== index));
												}}
											>
												<DeleteIcon classNames="opacity-0 group-hover:opacity-100 transition-all duration-250 rounded-md" />
											</div>
										)}
									</div>
								))}
						...
						<div className="flex gap-3">
							...
							<button type="submit">
								<SendIcon classNames="fill-foreground" />
							</button>
						</div>
					</div>
				</form>
