...
const PostComponent: React.FC<PostFunction> = ({ post, setParents }) => {
	...
	return (
		<div
			className="border border-foreground p-3 cursor-pointer"
			onClick={blockLink}
			onMouseEnter={() => hoverOuter(true)}
			onMouseLeave={() => hoverOuter(false)}
		>
			<div className="flex gap-3 justify-between">
				<div className="flex gap-2 items-center">
					<a
						href={`/account/${post.author?._id}`}
						className="text-lg font-bold"
						onClick={(event) => {
							event.stopPropagation();
						}}
						onMouseEnter={() => hoverInner(true)}
						onMouseLeave={() => hoverInner(false)}
					>
						{post.author?.username}
					</a>
					{post.date && (
						<p className="opacity-75 text-sm">{format(new Date(post.date), 'HH:mm, dd/MM/yyyy')}</p>
					)}
				</div>
			...
			<div
				className={`${outerHover && !innerHover && 'hover-hover:opacity-50'}`}
					...
					<div className="grid grid-cols-2 gap-2 my-3">
						{post.attachments.map((image, key) => (
							<Image
								...
								className="w-full h-auto border border-foreground rounded"
								width={600}
								height={400}
							/>
						))}
			...
			<div className="flex gap-3 justify-between">
				<div className="flex gap-3">
					...
					>
						<LikeIcon
							classNames={`${user && post.likes?.includes(user._id) ? 'fill-like' : 'fill-foreground'}`}
						/>
					...
					>
						<RepostIcon
							classNames={`${user && post.reposts?.includes(user._id) ? 'fill-repost' : 'fill-foreground'}`}
						/>
